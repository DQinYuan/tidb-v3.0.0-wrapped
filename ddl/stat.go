...
func (d *ddl) GetScope(status string) variable.ScopeFlag {
	trace_util_0.Count(_stat_00000, 0)
...
func (d *ddl) Stats(vars *variable.SessionVars) (map[string]interface{}, error) {
	trace_util_0.Count(_stat_00000, 1)
	m := make(map[string]interface{})
	m[serverID] = d.uuid
	var ddlInfo *admin.DDLInfo

	err := kv.RunInNewTxn(d.store, false, func(txn kv.Transaction) error {
		trace_util_0.Count(_stat_00000, 6)
		var err1 error
		ddlInfo, err1 = admin.GetDDLInfo(txn)
		if err1 != nil {
			trace_util_0.Count(_stat_00000, 8)
			return errors.Trace(err1)
		}
		trace_util_0.Count(_stat_00000, 7)
		return errors.Trace(err1)
	})
	trace_util_0.Count(_stat_00000, 2)
	if err != nil {
		trace_util_0.Count(_stat_00000, 9)
		return nil, errors.Trace(err)
	}

	trace_util_0.Count(_stat_00000, 3)
	m[ddlSchemaVersion] = ddlInfo.SchemaVer
	// TODO: Get the owner information.
	if len(ddlInfo.Jobs) == 0 {
		trace_util_0.Count(_stat_00000, 10)
		return m, nil
	}
	// TODO: Add all job information if needed.
	trace_util_0.Count(_stat_00000, 4)
	job := ddlInfo.Jobs[0]
	m[ddlJobID] = job.ID
	...
	if job.Error == nil {
		trace_util_0.Count(_stat_00000, 11)
		m[ddlJobError] = ""
	} else {
		trace_util_0.Count(_stat_00000, 12)
		{
			m[ddlJobError] = job.Error.Error()
		}
	}
	trace_util_0.Count(_stat_00000, 5)
