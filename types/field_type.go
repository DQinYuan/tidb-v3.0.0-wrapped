...
func NewFieldType(tp byte) *FieldType {
	trace_util_0.Count(_field_type_00000, 0)
	return &FieldType{
		Tp:      tp,
...
func AggFieldType(tps []*FieldType) *FieldType {
	trace_util_0.Count(_field_type_00000, 1)
	var currType FieldType
	for i, t := range tps {
		trace_util_0.Count(_field_type_00000, 3)
		if i == 0 && currType.Tp == mysql.TypeUnspecified {
			trace_util_0.Count(_field_type_00000, 5)
			currType = *t
			continue
		}
		trace_util_0.Count(_field_type_00000, 4)
		mtp := MergeFieldType(currType.Tp, t.Tp)
		currType.Tp = mtp
	}

	trace_util_0.Count(_field_type_00000, 2)
	return &currType
}
...
func AggregateEvalType(fts []*FieldType, flag *uint) EvalType {
	trace_util_0.Count(_field_type_00000, 6)
	var (
	...
	lft := fts[0]
	for _, ft := range fts {
		trace_util_0.Count(_field_type_00000, 8)
		if ft.Tp == mysql.TypeNull {
			trace_util_0.Count(_field_type_00000, 12)
			continue
		}
		trace_util_0.Count(_field_type_00000, 9)
		et := ft.EvalType()
		rft := ft
		if (IsTypeBlob(ft.Tp) || IsTypeVarchar(ft.Tp) || IsTypeChar(ft.Tp)) && mysql.HasBinaryFlag(ft.Flag) {
			trace_util_0.Count(_field_type_00000, 13)
			gotBinString = true
		}
		trace_util_0.Count(_field_type_00000, 10)
		if !gotFirst {
			trace_util_0.Count(_field_type_00000, 14)
			gotFirst = true
			aggregatedEvalType = et
			unsigned = mysql.HasUnsignedFlag(ft.Flag)
		} else {
			trace_util_0.Count(_field_type_00000, 15)
			{
		...
		trace_util_0.Count(_field_type_00000, 11)
		lft = rft
	}
	trace_util_0.Count(_field_type_00000, 7)
...
func mergeEvalType(lhs, rhs EvalType, lft, rft *FieldType, isLHSUnsigned, isRHSUnsigned bool) EvalType {
	trace_util_0.Count(_field_type_00000, 16)
	if lft.Tp == mysql.TypeUnspecified || rft.Tp == mysql.TypeUnspecified {
		trace_util_0.Count(_field_type_00000, 19)
		if lft.Tp == rft.Tp {
			trace_util_0.Count(_field_type_00000, 21)
			return ETString
		}
		trace_util_0.Count(_field_type_00000, 20)
		if lft.Tp == mysql.TypeUnspecified {
			trace_util_0.Count(_field_type_00000, 22)
			lhs = rhs
		} else {
			trace_util_0.Count(_field_type_00000, 23)
			{
				rhs = lhs
			}
		}
	}
	trace_util_0.Count(_field_type_00000, 17)
	if lhs.IsStringKind() || rhs.IsStringKind() {
		trace_util_0.Count(_field_type_00000, 24)
		return ETString
	} else {
		trace_util_0.Count(_field_type_00000, 25)
		if lhs == ETReal || rhs == ETReal {
			trace_util_0.Count(_field_type_00000, 26)
			return ETReal
		} else {
			trace_util_0.Count(_field_type_00000, 27)
			if lhs == ETDecimal || rhs == ETDecimal || isLHSUnsigned != isRHSUnsigned {
				trace_util_0.Count(_field_type_00000, 28)
				return ETDecimal
			}
		}
	}
	trace_util_0.Count(_field_type_00000, 18)
	return ETInt
}

func setTypeFlag(flag *uint, flagItem uint, on bool) {
	trace_util_0.Count(_field_type_00000, 29)
	if on {
		trace_util_0.Count(_field_type_00000, 30)
		*flag |= flagItem
	} else {
		trace_util_0.Count(_field_type_00000, 31)
		{
			*flag &= ^flagItem
		}
	}
...
func DefaultParamTypeForValue(value interface{}, tp *FieldType) {
	trace_util_0.Count(_field_type_00000, 32)
	switch value.(type) {
	case nil:
		trace_util_0.Count(_field_type_00000, 33)
		tp.Tp = mysql.TypeVarString
		tp.Flen = UnspecifiedLength
		tp.Decimal = UnspecifiedLength
	default:
		trace_util_0.Count(_field_type_00000, 34)
		DefaultTypeForValue(value, tp)
		if hasVariantFieldLength(tp) {
			trace_util_0.Count(_field_type_00000, 36)
			tp.Flen = UnspecifiedLength
		}
		trace_util_0.Count(_field_type_00000, 35)
		if tp.Tp == mysql.TypeUnspecified {
			trace_util_0.Count(_field_type_00000, 37)
			tp.Tp = mysql.TypeVarString
		}
	}
}
...
func hasVariantFieldLength(tp *FieldType) bool {
	trace_util_0.Count(_field_type_00000, 38)
	switch tp.Tp {
	case mysql.TypeLonglong, mysql.TypeVarString, mysql.TypeDouble, mysql.TypeBlob,
		...
		trace_util_0.Count(_field_type_00000, 40)
		return true
	}
	trace_util_0.Count(_field_type_00000, 39)
	return false
}

// DefaultTypeForValue returns the default FieldType for the value.
func DefaultTypeForValue(value interface{}, tp *FieldType) {
	trace_util_0.Count(_field_type_00000, 41)
	switch x := value.(type) {
	case nil:
		trace_util_0.Count(_field_type_00000, 42)
		tp.Tp = mysql.TypeNull
		tp.Flen = 0
		tp.Decimal = 0
		SetBinChsClnFlag(tp)
	case bool:
		trace_util_0.Count(_field_type_00000, 43)
		...
		SetBinChsClnFlag(tp)
	case int:
		trace_util_0.Count(_field_type_00000, 44)
		tp.Tp = mysql.TypeLonglong
		tp.Flen = utilMath.StrLenOfInt64Fast(int64(x))
		tp.Decimal = 0
		SetBinChsClnFlag(tp)
	case int64:
		trace_util_0.Count(_field_type_00000, 45)
		tp.Tp = mysql.TypeLonglong
		tp.Flen = utilMath.StrLenOfInt64Fast(x)
		tp.Decimal = 0
		SetBinChsClnFlag(tp)
	case uint64:
		trace_util_0.Count(_field_type_00000, 46)
		tp.Tp = mysql.TypeLonglong
		...
		SetBinChsClnFlag(tp)
	case string:
		trace_util_0.Count(_field_type_00000, 47)
		tp.Tp = mysql.TypeVarString
	...
	case float64:
		trace_util_0.Count(_field_type_00000, 48)
		...
		SetBinChsClnFlag(tp)
	case []byte:
		trace_util_0.Count(_field_type_00000, 49)
		tp.Tp = mysql.TypeBlob
		tp.Flen = len(x)
		tp.Decimal = UnspecifiedLength
		SetBinChsClnFlag(tp)
	case BitLiteral:
		trace_util_0.Count(_field_type_00000, 50)
		tp.Tp = mysql.TypeVarString
		tp.Flen = len(x)
		tp.Decimal = 0
		SetBinChsClnFlag(tp)
	case HexLiteral:
		trace_util_0.Count(_field_type_00000, 51)
		tp.Tp = mysql.TypeVarString
		tp.Flen = len(x)
		tp.Decimal = 0
		tp.Flag |= mysql.UnsignedFlag
		SetBinChsClnFlag(tp)
	case BinaryLiteral:
		trace_util_0.Count(_field_type_00000, 52)
		tp.Tp = mysql.TypeBit
		tp.Flen = len(x) * 8
		tp.Decimal = 0
		SetBinChsClnFlag(tp)
		...
		tp.Flag |= mysql.UnsignedFlag
	case Time:
		trace_util_0.Count(_field_type_00000, 53)
		tp.Tp = x.Type
		switch x.Type {
		case mysql.TypeDate:
			trace_util_0.Count(_field_type_00000, 62)
			tp.Flen = mysql.MaxDateWidth
			tp.Decimal = UnspecifiedLength
		case mysql.TypeDatetime, mysql.TypeTimestamp:
			trace_util_0.Count(_field_type_00000, 63)
			tp.Flen = mysql.MaxDatetimeWidthNoFsp
			if x.Fsp > DefaultFsp {
				trace_util_0.Count(_field_type_00000, 65) // consider point('.') and the fractional part.
				tp.Flen += x.Fsp + 1
			}
			trace_util_0.Count(_field_type_00000, 64)
			tp.Decimal = x.Fsp
		}
		trace_util_0.Count(_field_type_00000, 54)
		SetBinChsClnFlag(tp)
	case Duration:
		trace_util_0.Count(_field_type_00000, 55)
		tp.Tp = mysql.TypeDuration
		tp.Flen = len(x.String())
		if x.Fsp > DefaultFsp {
			trace_util_0.Count(_field_type_00000, 66) // consider point('.') and the fractional part.
			tp.Flen = x.Fsp + 1
		}
		trace_util_0.Count(_field_type_00000, 56)
		tp.Decimal = x.Fsp
		SetBinChsClnFlag(tp)
	case *MyDecimal:
		trace_util_0.Count(_field_type_00000, 57)
		tp.Tp = mysql.TypeNewDecimal
		tp.Flen = len(x.ToString())
		tp.Decimal = int(x.digitsFrac)
		SetBinChsClnFlag(tp)
	case Enum:
		trace_util_0.Count(_field_type_00000, 58)
		tp.Tp = mysql.TypeEnum
		tp.Flen = len(x.Name)
		tp.Decimal = UnspecifiedLength
		SetBinChsClnFlag(tp)
	case Set:
		trace_util_0.Count(_field_type_00000, 59)
		tp.Tp = mysql.TypeSet
		tp.Flen = len(x.Name)
		tp.Decimal = UnspecifiedLength
		SetBinChsClnFlag(tp)
	case json.BinaryJSON:
		trace_util_0.Count(_field_type_00000, 60)
	...
	default:
		trace_util_0.Count(_field_type_00000, 61)
...
func MergeFieldType(a byte, b byte) byte {
	trace_util_0.Count(_field_type_00000, 70)
...
func getFieldTypeIndex(tp byte) int {
	trace_util_0.Count(_field_type_00000, 71)
	itp := int(tp)
	if itp < fieldTypeTearFrom {
		trace_util_0.Count(_field_type_00000, 73)
		return itp
	}
	trace_util_0.Count(_field_type_00000, 72)
...
func SetBinChsClnFlag(ft *FieldType) {
	trace_util_0.Count(_field_type_00000, 74)
...
var _field_type_00000 = "types/field_type.go"
