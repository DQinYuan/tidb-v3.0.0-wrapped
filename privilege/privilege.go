...
func (k keyType) String() string {
	trace_util_0.Count(_privilege_00000, 0)
...
func BindPrivilegeManager(ctx sessionctx.Context, pc Manager) {
	trace_util_0.Count(_privilege_00000, 1)
	ctx.SetValue(key, pc)
}

// GetPrivilegeManager gets Checker from context.
func GetPrivilegeManager(ctx sessionctx.Context) Manager {
	trace_util_0.Count(_privilege_00000, 2)
	if v, ok := ctx.Value(key).(Manager); ok {
		trace_util_0.Count(_privilege_00000, 4)
		return v
	}
	trace_util_0.Count(_privilege_00000, 3)
	return nil
}

var _privilege_00000 = "privilege/privilege.go"
