...
func NewRangeTaskRunner(
	...
	trace_util_0.Count(_range_task_00000, 0)
...
func (s *RangeTaskRunner) SetStatLogInterval(interval time.Duration) {
	trace_util_0.Count(_range_task_00000, 1)
...
func (s *RangeTaskRunner) RunOnRange(ctx context.Context, startKey []byte, endKey []byte) error {
	trace_util_0.Count(_range_task_00000, 2)
	s.completedRegions = 0

	if len(endKey) != 0 && bytes.Compare(startKey, endKey) >= 0 {
		trace_util_0.Count(_range_task_00000, 8)
	...
	trace_util_0.Count(_range_task_00000, 3)
	...
	for i := 0; i < s.concurrency; i++ {
		trace_util_0.Count(_range_task_00000, 9)
	...
	trace_util_0.Count(_range_task_00000, 4)
	startTime := time.Now()

	// Make sure taskCh is closed exactly once
	isClosed := false
	defer func() {
		trace_util_0.Count(_range_task_00000, 10)
		if !isClosed {
			trace_util_0.Count(_range_task_00000, 12)
			close(taskCh)
			wg.Wait()
		}
		trace_util_0.Count(_range_task_00000, 11)
		statLogTicker.Stop()
		metricsTicker.Stop()
		cancel()
	}()
	...
	trace_util_0.Count(_range_task_00000, 5)
	key := startKey
	for {
		trace_util_0.Count(_range_task_00000, 13)
		select {
		case <-statLogTicker.C:
			trace_util_0.Count(_range_task_00000, 19)
		...
		trace_util_0.Count(_range_task_00000, 15)
		task := &kv.KeyRange{
			StartKey: key,
			EndKey:   loc.EndKey,
		}
		...
		if isLast {
			trace_util_0.Count(_range_task_00000, 23)
			task.EndKey = endKey
		}

		trace_util_0.Count(_range_task_00000, 16)
		pushTaskStartTime := time.Now()

		select {
		case taskCh <- task:
			trace_util_0.Count(_range_task_00000, 24)
		case <-ctx.Done():
			trace_util_0.Count(_range_task_00000, 25)
			break
		}
		trace_util_0.Count(_range_task_00000, 17)
		metrics.TiKVRangeTaskPushDuration.WithLabelValues(s.name).Observe(time.Since(pushTaskStartTime).Seconds())

		if isLast {
			trace_util_0.Count(_range_task_00000, 26)
			break
		}

		trace_util_0.Count(_range_task_00000, 18)
		key = task.EndKey
	}

	trace_util_0.Count(_range_task_00000, 6)
	isClosed = true
	close(taskCh)
	wg.Wait()
	for _, w := range workers {
		trace_util_0.Count(_range_task_00000, 27)
		if w.err != nil {
			trace_util_0.Count(_range_task_00000, 28)
			logutil.Logger(ctx).Info("range task failed",
				zap.String("name", s.name),
				zap.Binary("startKey", startKey),
				zap.Binary("endKey", endKey),
	...
	trace_util_0.Count(_range_task_00000, 7)
...
func (s *RangeTaskRunner) createWorker(taskCh chan *kv.KeyRange, wg *sync.WaitGroup) *rangeTaskWorker {
	trace_util_0.Count(_range_task_00000, 29)
...
func (s *RangeTaskRunner) CompletedRegions() int32 {
	trace_util_0.Count(_range_task_00000, 30)
...
func (w *rangeTaskWorker) run(ctx context.Context, cancel context.CancelFunc) {
	trace_util_0.Count(_range_task_00000, 31)
	defer w.wg.Done()

	for r := range w.taskCh {
		trace_util_0.Count(_range_task_00000, 32)
		select {
		case <-ctx.Done():
			trace_util_0.Count(_range_task_00000, 35)
			w.err = ctx.Err()
			break
		default:
			trace_util_0.Count(_range_task_00000, 36)
		}

		trace_util_0.Count(_range_task_00000, 33)
		completedRegions, err := w.handler(ctx, *r)
		if err != nil {
			trace_util_0.Count(_range_task_00000, 37)
		...
		trace_util_0.Count(_range_task_00000, 34)
		atomic.AddInt32(w.completedRegions, int32(completedRegions))
	}
}

var _range_task_00000 = "store/tikv/range_task.go"
