...
func IsTxnRetryableError(err error) bool {
	...
	if err == nil {
		trace_util_0.Count(_error_00000, 4)
		return false
	}

	trace_util_0.Count(_error_00000, 2)
	if ErrTxnRetryable.Equal(err) || ErrWriteConflict.Equal(err) || ErrWriteConflictInTiDB.Equal(err) {
		trace_util_0.Count(_error_00000, 5)
		return true
	}

	trace_util_0.Count(_error_00000, 3)
	return false
}

// IsErrNotFound checks if err is a kind of NotFound error.
func IsErrNotFound(err error) bool {
	trace_util_0.Count(_error_00000, 6)
	return ErrNotExist.Equal(err)
}

var _error_00000 = "kv/error.go"
