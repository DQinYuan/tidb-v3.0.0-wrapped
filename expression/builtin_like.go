...
func (c *likeFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	trace_util_0.Count(_builtin_like_00000, 0)
	if err := c.verifyArgs(args); err != nil {
		trace_util_0.Count(_builtin_like_00000, 2)
		return nil, err
	}
	trace_util_0.Count(_builtin_like_00000, 1)
...
func (b *builtinLikeSig) Clone() builtinFunc {
	trace_util_0.Count(_builtin_like_00000, 3)
...
func (c *regexpFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	trace_util_0.Count(_builtin_like_00000, 11)
	if err := c.verifyArgs(args); err != nil {
		trace_util_0.Count(_builtin_like_00000, 14)
		return nil, err
	}
	trace_util_0.Count(_builtin_like_00000, 12)
	bf := newBaseBuiltinFuncWithTp(ctx, args, types.ETInt, types.ETString, types.ETString)
	bf.tp.Flen = 1
	var sig builtinFunc
	if types.IsBinaryStr(args[0].GetType()) {
		trace_util_0.Count(_builtin_like_00000, 15)
		sig = &builtinRegexpBinarySig{bf}
	} else {
		trace_util_0.Count(_builtin_like_00000, 16)
		{
			sig = &builtinRegexpSig{bf}
		}
	}
	trace_util_0.Count(_builtin_like_00000, 13)
	return sig, nil
}
...
func (b *builtinRegexpBinarySig) Clone() builtinFunc {
	trace_util_0.Count(_builtin_like_00000, 17)
...
func (b *builtinRegexpBinarySig) evalInt(row chunk.Row) (int64, bool, error) {
	trace_util_0.Count(_builtin_like_00000, 18)
	expr, isNull, err := b.args[0].EvalString(b.ctx, row)
	if isNull || err != nil {
		trace_util_0.Count(_builtin_like_00000, 22)
		return 0, true, err
	}

	trace_util_0.Count(_builtin_like_00000, 19)
	pat, isNull, err := b.args[1].EvalString(b.ctx, row)
	if isNull || err != nil {
		trace_util_0.Count(_builtin_like_00000, 23)
		return 0, true, err
	}

	// TODO: We don't need to compile pattern if it has been compiled or it is static.
	trace_util_0.Count(_builtin_like_00000, 20)
	re, err := regexp.Compile(pat)
	if err != nil {
		trace_util_0.Count(_builtin_like_00000, 24)
		return 0, true, ErrRegexp.GenWithStackByArgs(err.Error())
	}
	trace_util_0.Count(_builtin_like_00000, 21)
...
func (b *builtinRegexpSig) Clone() builtinFunc {
	trace_util_0.Count(_builtin_like_00000, 25)
...
func (b *builtinRegexpSig) evalInt(row chunk.Row) (int64, bool, error) {
	trace_util_0.Count(_builtin_like_00000, 26)
	expr, isNull, err := b.args[0].EvalString(b.ctx, row)
	if isNull || err != nil {
		trace_util_0.Count(_builtin_like_00000, 30)
		return 0, true, err
	}

	trace_util_0.Count(_builtin_like_00000, 27)
	pat, isNull, err := b.args[1].EvalString(b.ctx, row)
	if isNull || err != nil {
		trace_util_0.Count(_builtin_like_00000, 31)
		return 0, true, err
	}

	// TODO: We don't need to compile pattern if it has been compiled or it is static.
	trace_util_0.Count(_builtin_like_00000, 28)
	re, err := regexp.Compile("(?i)" + pat)
	if err != nil {
		trace_util_0.Count(_builtin_like_00000, 32)
		return 0, true, ErrRegexp.GenWithStackByArgs(err.Error())
	}
	trace_util_0.Count(_builtin_like_00000, 29)
	return boolToInt64(re.MatchString(expr)), false, nil
}

var _builtin_like_00000 = "expression/builtin_like.go"
