...
func RegisterStatistics(s Statistics) {
	trace_util_0.Count(_statusvar_00000, 0)
...
func GetStatusVars(vars *SessionVars) (map[string]*StatusVal, error) {
	trace_util_0.Count(_statusvar_00000, 1)
	...
	for _, statistics := range statisticsList {
		trace_util_0.Count(_statusvar_00000, 3)
		vals, err := statistics.Stats(vars)
		if err != nil {
			trace_util_0.Count(_statusvar_00000, 5)
			return nil, err
		}

		trace_util_0.Count(_statusvar_00000, 4)
		for name, val := range vals {
			trace_util_0.Count(_statusvar_00000, 6)
			scope := statistics.GetScope(name)
			statusVars[name] = &StatusVal{Value: val, Scope: scope}
		}
	}

	trace_util_0.Count(_statusvar_00000, 2)
...
func (s defaultStatusStat) Stats(vars *SessionVars) (map[string]interface{}, error) {
	...
	trace_util_0.Count(_statusvar_00000, 10)
	return statusVars, nil
}

func init() {
	trace_util_0.Count(_statusvar_00000, 13)
	var ciphersBuffer bytes.Buffer
	for _, v := range tlsCiphers {
		trace_util_0.Count(_statusvar_00000, 15)
		ciphersBuffer.WriteString(tlsCipherString[v])
		ciphersBuffer.WriteString(":")
	}
	trace_util_0.Count(_statusvar_00000, 14)
...
var _statusvar_00000 = "sessionctx/variable/statusvar.go"
