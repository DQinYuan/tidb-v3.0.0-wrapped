...
func (a *stdAllocator) Alloc(capacity int) []byte {
	trace_util_0.Count(_arena_00000, 0)
	return make([]byte, 0, capacity)
}

func (a *stdAllocator) AllocWithLen(length int, capacity int) []byte {
	trace_util_0.Count(_arena_00000, 1)
	return make([]byte, length, capacity)
}

func (a *stdAllocator) Reset() {
	trace_util_0.Count(_arena_00000, 2)
}
...
func NewAllocator(capacity int) *SimpleAllocator {
	trace_util_0.Count(_arena_00000, 3)
...
func (s *SimpleAllocator) Reset() {
	trace_util_0.Count(_arena_00000, 8)
	s.off = 0
}

var _arena_00000 = "util/arena/arena.go"
