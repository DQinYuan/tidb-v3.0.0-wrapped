...
func (ts *PhysicalTableScan) IsPartition() (bool, int64) {
	trace_util_0.Count(_physical_plans_00000, 0)
...
func (at AggregationType) String() string {
	trace_util_0.Count(_physical_plans_00000, 1)
	switch at {
	case StreamedAgg:
		trace_util_0.Count(_physical_plans_00000, 3)
		return "stream"
	case FinalAgg:
		trace_util_0.Count(_physical_plans_00000, 4)
		return "final"
	case CompleteAgg:
		trace_util_0.Count(_physical_plans_00000, 5)
		return "complete"
	}
	trace_util_0.Count(_physical_plans_00000, 2)
...
func (p *basePhysicalAgg) hasDistinctFunc() bool {
	trace_util_0.Count(_physical_plans_00000, 6)
	for _, fun := range p.AggFuncs {
		trace_util_0.Count(_physical_plans_00000, 8)
		if fun.HasDistinct {
			trace_util_0.Count(_physical_plans_00000, 9)
			return true
		}
	}
	trace_util_0.Count(_physical_plans_00000, 7)
	return false
}
...
func (p *PhysicalIndexScan) IsPartition() (bool, int64) {
	trace_util_0.Count(_physical_plans_00000, 10)
...
func (p *PhysicalIndexScan) IsPointGetByUniqueKey(sc *stmtctx.StatementContext) bool {
	trace_util_0.Count(_physical_plans_00000, 11)
...
func CollectPlanStatsVersion(plan PhysicalPlan, statsInfos map[string]uint64) map[string]uint64 {
	trace_util_0.Count(_physical_plans_00000, 12)
	for _, child := range plan.Children() {
		trace_util_0.Count(_physical_plans_00000, 15)
		statsInfos = CollectPlanStatsVersion(child, statsInfos)
	}
	trace_util_0.Count(_physical_plans_00000, 13)
	switch copPlan := plan.(type) {
	case *PhysicalTableReader:
		trace_util_0.Count(_physical_plans_00000, 16)
		statsInfos = CollectPlanStatsVersion(copPlan.tablePlan, statsInfos)
	case *PhysicalIndexReader:
		trace_util_0.Count(_physical_plans_00000, 17)
		statsInfos = CollectPlanStatsVersion(copPlan.indexPlan, statsInfos)
	case *PhysicalIndexLookUpReader:
		trace_util_0.Count(_physical_plans_00000, 18)
	...
	case *PhysicalIndexScan:
		trace_util_0.Count(_physical_plans_00000, 19)
		statsInfos[copPlan.Table.Name.O] = copPlan.stats.StatsVersion
	case *PhysicalTableScan:
		trace_util_0.Count(_physical_plans_00000, 20)
		statsInfos[copPlan.Table.Name.O] = copPlan.stats.StatsVersion
	}

	trace_util_0.Count(_physical_plans_00000, 14)
	return statsInfos
}

var _physical_plans_00000 = "planner/core/physical_plans.go"
