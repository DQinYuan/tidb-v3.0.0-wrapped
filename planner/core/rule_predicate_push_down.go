// Copyright 2016 PingCAP, Inc.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
// // Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// See the License for the specific language governing permissions and
// limitations under the License.

package core

import (
	"github.com/pingcap/parser/ast"
	"github.com/pingcap/parser/model"
	"github.com/pingcap/parser/mysql"
	"github.com/pingcap/tidb/expression"
	"github.com/pingcap/tidb/sessionctx"
	"github.com/pingcap/tidb/trace_util_0"
	"github.com/pingcap/tidb/types"
)

type ppdSolver struct{}

func (s *ppdSolver) optimize(lp LogicalPlan) (LogicalPlan, error) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 0)
	_, p := lp.PredicatePushDown(nil)
	return p, nil
}

func addSelection(p LogicalPlan, child LogicalPlan, conditions []expression.Expression, chIdx int) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 1)
	if len(conditions) == 0 {
		trace_util_0.Count(_rule_predicate_push_down_00000, 4)
		p.Children()[chIdx] = child
		return
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 2)
	conditions = expression.PropagateConstant(p.context(), conditions)
	// Return table dual when filter is constant false or null.
	dual := conds2TableDual(child, conditions)
	if dual != nil {
		trace_util_0.Count(_rule_predicate_push_down_00000, 5)
		p.Children()[chIdx] = dual
		return
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 3)
	selection := LogicalSelection{Conditions: conditions}.Init(p.context())
	selection.SetChildren(child)
	p.Children()[chIdx] = selection
}

// PredicatePushDown implements LogicalPlan interface.
func (p *baseLogicalPlan) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 6)
	if len(p.children) == 0 {
		trace_util_0.Count(_rule_predicate_push_down_00000, 8)
		return predicates, p.self
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 7)
	child := p.children[0]
	rest, newChild := child.PredicatePushDown(predicates)
	addSelection(p.self, newChild, rest, 0)
	return nil, p.self
}

func splitSetGetVarFunc(filters []expression.Expression) ([]expression.Expression, []expression.Expression) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 9)
	canBePushDown := make([]expression.Expression, 0, len(filters))
	canNotBePushDown := make([]expression.Expression, 0, len(filters))
	for _, expr := range filters {
		trace_util_0.Count(_rule_predicate_push_down_00000, 11)
		if expression.HasGetSetVarFunc(expr) {
			trace_util_0.Count(_rule_predicate_push_down_00000, 12)
			canNotBePushDown = append(canNotBePushDown, expr)
		} else {
			trace_util_0.Count(_rule_predicate_push_down_00000, 13)
			{
				canBePushDown = append(canBePushDown, expr)
			}
		}
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 10)
	return canBePushDown, canNotBePushDown
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalSelection) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 14)
	canBePushDown, canNotBePushDown := splitSetGetVarFunc(p.Conditions)
	retConditions, child := p.children[0].PredicatePushDown(append(canBePushDown, predicates...))
	retConditions = append(retConditions, canNotBePushDown...)
	if len(retConditions) > 0 {
		trace_util_0.Count(_rule_predicate_push_down_00000, 16)
		p.Conditions = expression.PropagateConstant(p.ctx, retConditions)
		// Return table dual when filter is constant false or null.
		dual := conds2TableDual(p, p.Conditions)
		if dual != nil {
			trace_util_0.Count(_rule_predicate_push_down_00000, 18)
			return nil, dual
		}
		trace_util_0.Count(_rule_predicate_push_down_00000, 17)
		return nil, p
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 15)
	return nil, child
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalUnionScan) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 19)
	retainedPredicates, _ := p.children[0].PredicatePushDown(predicates)
	p.conditions = make([]expression.Expression, 0, len(predicates))
	p.conditions = append(p.conditions, predicates...)
	// The conditions in UnionScan is only used for added rows, so parent Selection should not be removed.
	return retainedPredicates, p
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (ds *DataSource) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 20)
	ds.allConds = predicates
	_, ds.pushedDownConds, predicates = expression.ExpressionsToPB(ds.ctx.GetSessionVars().StmtCtx, predicates, ds.ctx.GetClient())
	return predicates, ds
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalTableDual) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 21)
	return predicates, p
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalJoin) PredicatePushDown(predicates []expression.Expression) (ret []expression.Expression, retPlan LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 22)
	simplifyOuterJoin(p, predicates)
	var equalCond []*expression.ScalarFunction
	var leftPushCond, rightPushCond, otherCond, leftCond, rightCond []expression.Expression
	switch p.JoinType {
	case LeftOuterJoin, LeftOuterSemiJoin, AntiLeftOuterSemiJoin:
		trace_util_0.Count(_rule_predicate_push_down_00000, 25)
		predicates = p.outerJoinPropConst(predicates)
		dual := conds2TableDual(p, predicates)
		if dual != nil {
			trace_util_0.Count(_rule_predicate_push_down_00000, 31)
			return ret, dual
		}
		// Handle where conditions
		trace_util_0.Count(_rule_predicate_push_down_00000, 26)
		predicates = expression.ExtractFiltersFromDNFs(p.ctx, predicates)
		// Only derive left where condition, because right where condition cannot be pushed down
		equalCond, leftPushCond, rightPushCond, otherCond = p.extractOnCondition(predicates, true, false)
		leftCond = leftPushCond
		// Handle join conditions, only derive right join condition, because left join condition cannot be pushed down
		_, derivedRightJoinCond := deriveOtherConditions(p, false, true)
		rightCond = append(p.RightConditions, derivedRightJoinCond...)
		p.RightConditions = nil
		ret = append(expression.ScalarFuncs2Exprs(equalCond), otherCond...)
		ret = append(ret, rightPushCond...)
	case RightOuterJoin:
		trace_util_0.Count(_rule_predicate_push_down_00000, 27)
		predicates = p.outerJoinPropConst(predicates)
		dual := conds2TableDual(p, predicates)
		if dual != nil {
			trace_util_0.Count(_rule_predicate_push_down_00000, 32)
			return ret, dual
		}
		// Handle where conditions
		trace_util_0.Count(_rule_predicate_push_down_00000, 28)
		predicates = expression.ExtractFiltersFromDNFs(p.ctx, predicates)
		// Only derive right where condition, because left where condition cannot be pushed down
		equalCond, leftPushCond, rightPushCond, otherCond = p.extractOnCondition(predicates, false, true)
		rightCond = rightPushCond
		// Handle join conditions, only derive left join condition, because right join condition cannot be pushed down
		derivedLeftJoinCond, _ := deriveOtherConditions(p, true, false)
		leftCond = append(p.LeftConditions, derivedLeftJoinCond...)
		p.LeftConditions = nil
		ret = append(expression.ScalarFuncs2Exprs(equalCond), otherCond...)
		ret = append(ret, leftPushCond...)
	case SemiJoin, AntiSemiJoin, InnerJoin:
		trace_util_0.Count(_rule_predicate_push_down_00000, 29)
		tempCond := make([]expression.Expression, 0, len(p.LeftConditions)+len(p.RightConditions)+len(p.EqualConditions)+len(p.OtherConditions)+len(predicates))
		tempCond = append(tempCond, p.LeftConditions...)
		tempCond = append(tempCond, p.RightConditions...)
		tempCond = append(tempCond, expression.ScalarFuncs2Exprs(p.EqualConditions)...)
		tempCond = append(tempCond, p.OtherConditions...)
		tempCond = append(tempCond, predicates...)
		tempCond = expression.ExtractFiltersFromDNFs(p.ctx, tempCond)
		tempCond = expression.PropagateConstant(p.ctx, tempCond)
		// Return table dual when filter is constant false or null. Not applicable to AntiSemiJoin.
		// TODO: For AntiSemiJoin, we can use outer plan to substitute LogicalJoin actually.
		if p.JoinType != AntiSemiJoin {
			trace_util_0.Count(_rule_predicate_push_down_00000, 33)
			dual := conds2TableDual(p, tempCond)
			if dual != nil {
				trace_util_0.Count(_rule_predicate_push_down_00000, 34)
				return ret, dual
			}
		}
		trace_util_0.Count(_rule_predicate_push_down_00000, 30)
		equalCond, leftPushCond, rightPushCond, otherCond = p.extractOnCondition(tempCond, true, true)
		p.LeftConditions = nil
		p.RightConditions = nil
		p.EqualConditions = equalCond
		p.OtherConditions = otherCond
		leftCond = leftPushCond
		rightCond = rightPushCond
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 23)
	leftCond = expression.RemoveDupExprs(p.ctx, leftCond)
	rightCond = expression.RemoveDupExprs(p.ctx, rightCond)
	leftRet, lCh := p.children[0].PredicatePushDown(leftCond)
	rightRet, rCh := p.children[1].PredicatePushDown(rightCond)
	addSelection(p, lCh, leftRet, 0)
	addSelection(p, rCh, rightRet, 1)
	p.updateEQCond()
	for _, eqCond := range p.EqualConditions {
		trace_util_0.Count(_rule_predicate_push_down_00000, 35)
		p.LeftJoinKeys = append(p.LeftJoinKeys, eqCond.GetArgs()[0].(*expression.Column))
		p.RightJoinKeys = append(p.RightJoinKeys, eqCond.GetArgs()[1].(*expression.Column))
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 24)
	p.mergeSchema()
	p.buildKeyInfo()
	return ret, p.self
}

// updateEQCond will extract the arguments of a equal condition that connect two expressions.
func (p *LogicalJoin) updateEQCond() {
	trace_util_0.Count(_rule_predicate_push_down_00000, 36)
	lChild, rChild := p.children[0], p.children[1]
	var lKeys, rKeys []expression.Expression
	for i := len(p.OtherConditions) - 1; i >= 0; i-- {
		trace_util_0.Count(_rule_predicate_push_down_00000, 38)
		need2Remove := false
		if eqCond, ok := p.OtherConditions[i].(*expression.ScalarFunction); ok && eqCond.FuncName.L == ast.EQ {
			trace_util_0.Count(_rule_predicate_push_down_00000, 40)
			// If it is a column equal condition converted from `[not] in (subq)`, do not move it
			// to EqualConditions, and keep it in OtherConditions. Reference comments in `extractOnCondition`
			// for detailed reasons.
			if expression.IsEQCondFromIn(eqCond) {
				trace_util_0.Count(_rule_predicate_push_down_00000, 42)
				continue
			}
			trace_util_0.Count(_rule_predicate_push_down_00000, 41)
			lExpr, rExpr := eqCond.GetArgs()[0], eqCond.GetArgs()[1]
			if expression.ExprFromSchema(lExpr, lChild.Schema()) && expression.ExprFromSchema(rExpr, rChild.Schema()) {
				trace_util_0.Count(_rule_predicate_push_down_00000, 43)
				lKeys = append(lKeys, lExpr)
				rKeys = append(rKeys, rExpr)
				need2Remove = true
			} else {
				trace_util_0.Count(_rule_predicate_push_down_00000, 44)
				if expression.ExprFromSchema(lExpr, rChild.Schema()) && expression.ExprFromSchema(rExpr, lChild.Schema()) {
					trace_util_0.Count(_rule_predicate_push_down_00000, 45)
					lKeys = append(lKeys, rExpr)
					rKeys = append(rKeys, lExpr)
					need2Remove = true
				}
			}
		}
		trace_util_0.Count(_rule_predicate_push_down_00000, 39)
		if need2Remove {
			trace_util_0.Count(_rule_predicate_push_down_00000, 46)
			p.OtherConditions = append(p.OtherConditions[:i], p.OtherConditions[i+1:]...)
		}
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 37)
	if len(lKeys) > 0 {
		trace_util_0.Count(_rule_predicate_push_down_00000, 47)
		lProj := p.getProj(0)
		rProj := p.getProj(1)
		for i := range lKeys {
			trace_util_0.Count(_rule_predicate_push_down_00000, 48)
			lKey := lProj.appendExpr(lKeys[i])
			rKey := rProj.appendExpr(rKeys[i])
			eqCond := expression.NewFunctionInternal(p.ctx, ast.EQ, types.NewFieldType(mysql.TypeTiny), lKey, rKey)
			p.EqualConditions = append(p.EqualConditions, eqCond.(*expression.ScalarFunction))
		}
	}
}

func (p *LogicalProjection) appendExpr(expr expression.Expression) *expression.Column {
	trace_util_0.Count(_rule_predicate_push_down_00000, 49)
	if col, ok := expr.(*expression.Column); ok {
		trace_util_0.Count(_rule_predicate_push_down_00000, 51)
		return col
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 50)
	expr = expression.ColumnSubstitute(expr, p.schema, p.Exprs)
	p.Exprs = append(p.Exprs, expr)

	col := &expression.Column{
		UniqueID: p.ctx.GetSessionVars().AllocPlanColumnID(),
		ColName:  model.NewCIStr(expr.String()),
		RetType:  expr.GetType(),
	}
	p.schema.Append(col)
	return col
}

func (p *LogicalJoin) getProj(idx int) *LogicalProjection {
	trace_util_0.Count(_rule_predicate_push_down_00000, 52)
	child := p.children[idx]
	proj, ok := child.(*LogicalProjection)
	if ok {
		trace_util_0.Count(_rule_predicate_push_down_00000, 55)
		return proj
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 53)
	proj = LogicalProjection{Exprs: make([]expression.Expression, 0, child.Schema().Len())}.Init(p.ctx)
	for _, col := range child.Schema().Columns {
		trace_util_0.Count(_rule_predicate_push_down_00000, 56)
		proj.Exprs = append(proj.Exprs, col)
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 54)
	proj.SetSchema(child.Schema().Clone())
	proj.SetChildren(child)
	p.children[idx] = proj
	return proj
}

// simplifyOuterJoin transforms "LeftOuterJoin/RightOuterJoin" to "InnerJoin" if possible.
func simplifyOuterJoin(p *LogicalJoin, predicates []expression.Expression) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 57)
	if p.JoinType != LeftOuterJoin && p.JoinType != RightOuterJoin && p.JoinType != InnerJoin {
		trace_util_0.Count(_rule_predicate_push_down_00000, 64)
		return
	}

	trace_util_0.Count(_rule_predicate_push_down_00000, 58)
	innerTable := p.children[0]
	outerTable := p.children[1]
	if p.JoinType == LeftOuterJoin {
		trace_util_0.Count(_rule_predicate_push_down_00000, 65)
		innerTable, outerTable = outerTable, innerTable
	}

	// first simplify embedded outer join.
	trace_util_0.Count(_rule_predicate_push_down_00000, 59)
	if innerPlan, ok := innerTable.(*LogicalJoin); ok {
		trace_util_0.Count(_rule_predicate_push_down_00000, 66)
		simplifyOuterJoin(innerPlan, predicates)
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 60)
	if outerPlan, ok := outerTable.(*LogicalJoin); ok {
		trace_util_0.Count(_rule_predicate_push_down_00000, 67)
		simplifyOuterJoin(outerPlan, predicates)
	}

	trace_util_0.Count(_rule_predicate_push_down_00000, 61)
	if p.JoinType == InnerJoin {
		trace_util_0.Count(_rule_predicate_push_down_00000, 68)
		return
	}
	// then simplify embedding outer join.
	trace_util_0.Count(_rule_predicate_push_down_00000, 62)
	canBeSimplified := false
	for _, expr := range predicates {
		trace_util_0.Count(_rule_predicate_push_down_00000, 69)
		isOk := isNullRejected(p.ctx, innerTable.Schema(), expr)
		if isOk {
			trace_util_0.Count(_rule_predicate_push_down_00000, 70)
			canBeSimplified = true
			break
		}
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 63)
	if canBeSimplified {
		trace_util_0.Count(_rule_predicate_push_down_00000, 71)
		p.JoinType = InnerJoin
	}
}

// isNullRejected check whether a condition is null-rejected
// A condition would be null-rejected in one of following cases:
// If it is a predicate containing a reference to an inner table that evaluates to UNKNOWN or FALSE when one of its arguments is NULL.
// If it is a conjunction containing a null-rejected condition as a conjunct.
// If it is a disjunction of null-rejected conditions.
func isNullRejected(ctx sessionctx.Context, schema *expression.Schema, expr expression.Expression) bool {
	trace_util_0.Count(_rule_predicate_push_down_00000, 72)
	expr = expression.PushDownNot(nil, expr, false)
	sc := ctx.GetSessionVars().StmtCtx
	sc.InNullRejectCheck = true
	result := expression.EvaluateExprWithNull(ctx, schema, expr)
	sc.InNullRejectCheck = false
	x, ok := result.(*expression.Constant)
	if !ok {
		trace_util_0.Count(_rule_predicate_push_down_00000, 75)
		return false
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 73)
	if x.Value.IsNull() {
		trace_util_0.Count(_rule_predicate_push_down_00000, 76)
		return true
	} else {
		trace_util_0.Count(_rule_predicate_push_down_00000, 77)
		if isTrue, err := x.Value.ToBool(sc); err == nil && isTrue == 0 {
			trace_util_0.Count(_rule_predicate_push_down_00000, 78)
			return true
		}
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 74)
	return false
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalProjection) PredicatePushDown(predicates []expression.Expression) (ret []expression.Expression, retPlan LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 79)
	canBePushed := make([]expression.Expression, 0, len(predicates))
	canNotBePushed := make([]expression.Expression, 0, len(predicates))
	for _, cond := range predicates {
		trace_util_0.Count(_rule_predicate_push_down_00000, 81)
		newFilter := expression.ColumnSubstitute(cond, p.Schema(), p.Exprs)
		if !expression.HasGetSetVarFunc(newFilter) {
			trace_util_0.Count(_rule_predicate_push_down_00000, 82)
			canBePushed = append(canBePushed, expression.ColumnSubstitute(cond, p.Schema(), p.Exprs))
		} else {
			trace_util_0.Count(_rule_predicate_push_down_00000, 83)
			{
				canNotBePushed = append(canNotBePushed, cond)
			}
		}
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 80)
	remained, child := p.baseLogicalPlan.PredicatePushDown(canBePushed)
	return append(remained, canNotBePushed...), child
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalUnionAll) PredicatePushDown(predicates []expression.Expression) (ret []expression.Expression, retPlan LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 84)
	for i, proj := range p.children {
		trace_util_0.Count(_rule_predicate_push_down_00000, 86)
		newExprs := make([]expression.Expression, 0, len(predicates))
		newExprs = append(newExprs, predicates...)
		retCond, newChild := proj.PredicatePushDown(newExprs)
		addSelection(p, newChild, retCond, i)
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 85)
	return nil, p
}

// getGbyColIndex gets the column's index in the group-by columns.
func (la *LogicalAggregation) getGbyColIndex(col *expression.Column) int {
	trace_util_0.Count(_rule_predicate_push_down_00000, 87)
	return expression.NewSchema(la.groupByCols...).ColumnIndex(col)
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (la *LogicalAggregation) PredicatePushDown(predicates []expression.Expression) (ret []expression.Expression, retPlan LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 88)
	var condsToPush []expression.Expression
	exprsOriginal := make([]expression.Expression, 0, len(la.AggFuncs))
	for _, fun := range la.AggFuncs {
		trace_util_0.Count(_rule_predicate_push_down_00000, 91)
		exprsOriginal = append(exprsOriginal, fun.Args[0])
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 89)
	for _, cond := range predicates {
		trace_util_0.Count(_rule_predicate_push_down_00000, 92)
		switch cond.(type) {
		case *expression.Constant:
			trace_util_0.Count(_rule_predicate_push_down_00000, 93)
			condsToPush = append(condsToPush, cond)
			// Consider SQL list "select sum(b) from t group by a having 1=0". "1=0" is a constant predicate which should be
			// retained and pushed down at the same time. Because we will get a wrong query result that contains one column
			// with value 0 rather than an empty query result.
			ret = append(ret, cond)
		case *expression.ScalarFunction:
			trace_util_0.Count(_rule_predicate_push_down_00000, 94)
			extractedCols := expression.ExtractColumns(cond)
			ok := true
			for _, col := range extractedCols {
				trace_util_0.Count(_rule_predicate_push_down_00000, 97)
				if la.getGbyColIndex(col) == -1 {
					trace_util_0.Count(_rule_predicate_push_down_00000, 98)
					ok = false
					break
				}
			}
			trace_util_0.Count(_rule_predicate_push_down_00000, 95)
			if ok {
				trace_util_0.Count(_rule_predicate_push_down_00000, 99)
				newFunc := expression.ColumnSubstitute(cond, la.Schema(), exprsOriginal)
				condsToPush = append(condsToPush, newFunc)
			} else {
				trace_util_0.Count(_rule_predicate_push_down_00000, 100)
				{
					ret = append(ret, cond)
				}
			}
		default:
			trace_util_0.Count(_rule_predicate_push_down_00000, 96)
			ret = append(ret, cond)
		}
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 90)
	la.baseLogicalPlan.PredicatePushDown(condsToPush)
	return ret, la
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalLimit) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 101)
	// Limit forbids any condition to push down.
	p.baseLogicalPlan.PredicatePushDown(nil)
	return predicates, p
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalMaxOneRow) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 102)
	// MaxOneRow forbids any condition to push down.
	p.baseLogicalPlan.PredicatePushDown(nil)
	return predicates, p
}

// deriveOtherConditions given a LogicalJoin, check the OtherConditions to see if we can derive more
// conditions for left/right child pushdown.
func deriveOtherConditions(p *LogicalJoin, deriveLeft bool, deriveRight bool) (leftCond []expression.Expression,
	rightCond []expression.Expression) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 103)
	leftPlan, rightPlan := p.children[0], p.children[1]
	isOuterSemi := (p.JoinType == LeftOuterSemiJoin) || (p.JoinType == AntiLeftOuterSemiJoin)
	for _, expr := range p.OtherConditions {
		trace_util_0.Count(_rule_predicate_push_down_00000, 105)
		if deriveLeft {
			trace_util_0.Count(_rule_predicate_push_down_00000, 107)
			leftRelaxedCond := expression.DeriveRelaxedFiltersFromDNF(expr, leftPlan.Schema())
			if leftRelaxedCond != nil {
				trace_util_0.Count(_rule_predicate_push_down_00000, 109)
				leftCond = append(leftCond, leftRelaxedCond)
			}
			trace_util_0.Count(_rule_predicate_push_down_00000, 108)
			notNullExpr := deriveNotNullExpr(expr, leftPlan.Schema())
			if notNullExpr != nil {
				trace_util_0.Count(_rule_predicate_push_down_00000, 110)
				leftCond = append(leftCond, notNullExpr)
			}
		}
		trace_util_0.Count(_rule_predicate_push_down_00000, 106)
		if deriveRight {
			trace_util_0.Count(_rule_predicate_push_down_00000, 111)
			rightRelaxedCond := expression.DeriveRelaxedFiltersFromDNF(expr, rightPlan.Schema())
			if rightRelaxedCond != nil {
				trace_util_0.Count(_rule_predicate_push_down_00000, 114)
				rightCond = append(rightCond, rightRelaxedCond)
			}
			// For LeftOuterSemiJoin and AntiLeftOuterSemiJoin, we can actually generate
			// `col is not null` according to expressions in `OtherConditions` now, but we
			// are putting column equal condition converted from `in (subq)` into
			// `OtherConditions`(@sa https://github.com/pingcap/tidb/pull/9051), then it would
			// cause wrong results, so we disable this optimization for outer semi joins now.
			// TODO enable this optimization for outer semi joins later by checking whether
			// condition in `OtherConditions` is converted from `in (subq)`.
			trace_util_0.Count(_rule_predicate_push_down_00000, 112)
			if isOuterSemi {
				trace_util_0.Count(_rule_predicate_push_down_00000, 115)
				continue
			}
			trace_util_0.Count(_rule_predicate_push_down_00000, 113)
			notNullExpr := deriveNotNullExpr(expr, rightPlan.Schema())
			if notNullExpr != nil {
				trace_util_0.Count(_rule_predicate_push_down_00000, 116)
				rightCond = append(rightCond, notNullExpr)
			}
		}
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 104)
	return
}

// deriveNotNullExpr generates a new expression `not(isnull(col))` given `col1 op col2`,
// in which `col` is in specified schema. Caller guarantees that only one of `col1` or
// `col2` is in schema.
func deriveNotNullExpr(expr expression.Expression, schema *expression.Schema) expression.Expression {
	trace_util_0.Count(_rule_predicate_push_down_00000, 117)
	binop, ok := expr.(*expression.ScalarFunction)
	if !ok || len(binop.GetArgs()) != 2 {
		trace_util_0.Count(_rule_predicate_push_down_00000, 122)
		return nil
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 118)
	ctx := binop.GetCtx()
	arg0, lOK := binop.GetArgs()[0].(*expression.Column)
	arg1, rOK := binop.GetArgs()[1].(*expression.Column)
	if !lOK || !rOK {
		trace_util_0.Count(_rule_predicate_push_down_00000, 123)
		return nil
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 119)
	childCol := schema.RetrieveColumn(arg0)
	if childCol == nil {
		trace_util_0.Count(_rule_predicate_push_down_00000, 124)
		childCol = schema.RetrieveColumn(arg1)
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 120)
	if isNullRejected(ctx, schema, expr) && !mysql.HasNotNullFlag(childCol.RetType.Flag) {
		trace_util_0.Count(_rule_predicate_push_down_00000, 125)
		return expression.BuildNotNullExpr(ctx, childCol)
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 121)
	return nil
}

// conds2TableDual builds a LogicalTableDual if cond is constant false or null.
func conds2TableDual(p LogicalPlan, conds []expression.Expression) LogicalPlan {
	trace_util_0.Count(_rule_predicate_push_down_00000, 126)
	if len(conds) != 1 {
		trace_util_0.Count(_rule_predicate_push_down_00000, 130)
		return nil
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 127)
	con, ok := conds[0].(*expression.Constant)
	if !ok {
		trace_util_0.Count(_rule_predicate_push_down_00000, 131)
		return nil
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 128)
	sc := p.context().GetSessionVars().StmtCtx
	if isTrue, err := con.Value.ToBool(sc); (err == nil && isTrue == 0) || con.Value.IsNull() {
		trace_util_0.Count(_rule_predicate_push_down_00000, 132)
		dual := LogicalTableDual{}.Init(p.context())
		dual.SetSchema(p.Schema())
		return dual
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 129)
	return nil
}

// outerJoinPropConst propagates constant equal and column equal conditions over outer join.
func (p *LogicalJoin) outerJoinPropConst(predicates []expression.Expression) []expression.Expression {
	trace_util_0.Count(_rule_predicate_push_down_00000, 133)
	outerTable := p.children[0]
	innerTable := p.children[1]
	if p.JoinType == RightOuterJoin {
		trace_util_0.Count(_rule_predicate_push_down_00000, 136)
		innerTable, outerTable = outerTable, innerTable
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 134)
	lenJoinConds := len(p.EqualConditions) + len(p.LeftConditions) + len(p.RightConditions) + len(p.OtherConditions)
	joinConds := make([]expression.Expression, 0, lenJoinConds)
	for _, equalCond := range p.EqualConditions {
		trace_util_0.Count(_rule_predicate_push_down_00000, 137)
		joinConds = append(joinConds, equalCond)
	}
	trace_util_0.Count(_rule_predicate_push_down_00000, 135)
	joinConds = append(joinConds, p.LeftConditions...)
	joinConds = append(joinConds, p.RightConditions...)
	joinConds = append(joinConds, p.OtherConditions...)
	p.EqualConditions = nil
	p.LeftConditions = nil
	p.RightConditions = nil
	p.OtherConditions = nil
	nullSensitive := (p.JoinType == AntiLeftOuterSemiJoin || p.JoinType == LeftOuterSemiJoin)
	joinConds, predicates = expression.PropConstOverOuterJoin(p.ctx, joinConds, predicates, outerTable.Schema(), innerTable.Schema(), nullSensitive)
	p.attachOnConds(joinConds)
	return predicates
}

// PredicatePushDown implements LogicalPlan PredicatePushDown interface.
func (p *LogicalWindow) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	trace_util_0.Count(_rule_predicate_push_down_00000, 138)
	// Window function forbids any condition to push down.
	p.baseLogicalPlan.PredicatePushDown(nil)
	return predicates, p
}

var _rule_predicate_push_down_00000 = "planner/core/rule_predicate_push_down.go"
